...
class _UploadDataState extends State<UploadData> {
  ...
  Future<void> uploadDataToDatabase(List<List<dynamic>> data) async {
    ...
    // Function to batch upload 100 rows at a time
    Future<void> batchUpload(List<Map<String, dynamic>> batchData) async {
      Map<String, dynamic> updates = {};
      for (var data in batchData) {
        String key = journeyRef.push().key!;
        updates[key] = data;
      }

      try {
        await journeyRef.update(updates);
        print("Batch upload berhasil.");
      } catch (error) {
        print("Gagal melakukan batch upload: $error");
      }
    }
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      batchData.add(rowMap);

      // If we have 100 rows or it’s the last batch, upload them
      if (batchData.length == 250 || i == data.length - 1) {
        await batchUpload(batchData);
        batchData.clear(); // Clear the batch data for the next batch
      }
    }
  }
  ...
  // Future<void> uploadDataToDatabaseInBatches(List<List<dynamic>> data, int batchSize) async {
  //   int currentJourney = await getCurrentJourneyNumber();
  //   int newJourney = currentJourney + 1;
  //   String tanggalUpload = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
  //
  //   DatabaseReference journeyRef = userRef.child(widget.plat).child('Data Perjalanan $newJourney');
  //
  //   try {
  ...
  //     List<List<dynamic>> batch = data.sublist(
  //         i,
  //         i + batchSize > data.length ? data.length : i + batchSize
  //     );
  //
  //     print("Uploading batch starting from row $i");
  //
  //     try {
  //       // Upload the current batch
  //       await uploadBatchToDatabase(journeyRef, batch, i);
  //       print("Batch starting from row $i successfully uploaded.");
  //     } catch (error) {
  //       print("Failed to upload batch starting from row $i: $error");
  //     }
  //   }
  // }

  // Future<void> uploadBatchToDatabase(DatabaseReference journeyRef, List<List<dynamic>> batch, int startIndex) async {
  //   for (int i = 0; i < batch.length; i++) {
  //     List<dynamic> rowData = batch[i];
  //     if (rowData.length < 11) {
  //       print("Data tidak lengkap pada baris ${startIndex + i}: $rowData");
  //       continue; // Skip this row
  //     }
  //
  //     String datetime = rowData[0].toString();
  ...
  //         'accel_data_z': accel_data_z,
  //       });
  //       print("Data pada baris ${startIndex + i} berhasil diupload ke database.");
  //     } catch (error) {
  //       print("Gagal mengupload data pada baris ${startIndex + i}: $error");
  //     }
  //   }
  // }
  //
  ...
  //       });
  //
  //       await uploadDataToDatabaseInBatches(selectedData, 500);
  //
  //       setState(() {
  //         isLoadingUpload = false;
  //       });
