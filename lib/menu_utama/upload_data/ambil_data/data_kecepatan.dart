...
class DataKecepatan extends StatefulWidget {
  @override
  final String email;

  const DataKecepatan({Key? key, required this.email}) : super(key: key);
  State<DataKecepatan> createState() => _DataKecepatanState();
}

class _DataKecepatanState extends State<DataKecepatan> {
  ...
  Future<void> uploadDataToDatabase(List<List<dynamic>> data) async {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      double speed = double.tryParse(rowData[1].toString()) ?? 0.0;

      // String fullDateTime = '$date, $time';

      try {
        await databaseRef.child('DataKecepatan').push().set({
          'speed': speed,
          'datetime': datetime,
          'plat' : email, //Menambahkan email ke data yang diunggah
        });
        print('$speed, $datetime');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Data Kecepatan", style: TextStyle(color: Colors.black87)),
        actions: [
          IconButton(
            ...
            return Padding(
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  title: Text(
                    "${_csvTable[index][1].toString()} Km/h",
                    style: TextStyle(fontWeight: FontWeight.bold),
                  ),
                  subtitle: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Datetime: ${_csvTable[index][0].toString()}",style: TextStyle(color: Colors.black),),
                      // Text("Time: ${_csvTable[index][1].toString()}"),
                    ],
                  ),
                ),
              ),
