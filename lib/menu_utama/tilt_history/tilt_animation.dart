...
class _TiltAnimationState extends State<TiltAnimation> {
  ...
  List<List<double>> _filteredCsvTable = [];
  int currentIndex = 0;
  ...
  void _applyDateFilter() {
    if (selectedDateRange != null) {
      final startTimestamp = selectedDateRange!.start.millisecondsSinceEpoch.toDouble();
      final endTimestamp = selectedDateRange!.end.millisecondsSinceEpoch.toDouble();

      _filteredCsvTable = _csvTable.where((data) {
        final timestamp = data[0];
        return timestamp >= startTimestamp && timestamp <= endTimestamp;
      }).toList();
    } else {
      _filteredCsvTable = List.from(_csvTable);
    }
    currentIndex = 0; // Reset currentIndex after applying filter
  }

  Future<void> _showDateRangePicker() async {
    if (_csvTable.isEmpty) return;

    final firstDate = DateTime.fromMillisecondsSinceEpoch(_csvTable.first[0].toInt());
    final lastDate = DateTime.fromMillisecondsSinceEpoch(_csvTable.last[0].toInt());

    final picked = await showOmniDateTimeRangePicker(
      context: context,
      startInitialDate: firstDate,
      startFirstDate: firstDate,
      startLastDate: lastDate,
      endInitialDate: lastDate,
      endFirstDate: firstDate,
      endLastDate: lastDate,
      is24HourMode: true,
      isShowSeconds: false,
      minutesInterval: 1,
      secondsInterval: 1,
    ...
    if (picked != null && picked.length == 2) {
      setState(() {
        selectedDateRange = DateTimeRange(start: picked[0], end: picked[1]);
        _applyDateFilter();
      });
    }
  }
          ...
          child: Card(
            child: Column(
              children: [
                SizedBox(height: 10,),
                Text(
                  // 'Visualisasi Animasi Kendaraan',
                  'Vehicle Animation Visualization',
                  style: TextStyle(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: TextButton(
                          ...
                          child:
                          Icon(Icons.filter_list, color: texticon, size: 18,)
                      ),
                    ),
                    SizedBox(width: 10,)
                  ],
